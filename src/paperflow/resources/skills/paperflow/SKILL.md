...
Run `paperflow triage <citekey>` for one paper or `paperflow triage` for the backlog. The command prepares `.paperflow/triage/<zotero-key>/packet.md` from the Literature note — frontmatter, metadata, abstract, Zotero annotations, section titles when the paper is already ingested, and a digest of the Vault's Research Areas, open Questions, and existing Literature. Then read `prompts/triage.md` and write `paper-card.md` beside the packet. Rerun `paperflow triage` to validate the card and see priorities; a failed card exits nonzero with the reason.

A card is a routing decision, not a reading: `research_area`, `priority`, `deep_processing`, `basis`, and five sections under 80 words each. Do not write a summary, do not read `source.md`, and do not assert anything the packet does not state. Triage is offline — no Zotero connection and no PDF converter — so a large backlog costs one bounded packet per paper.

Only papers a card marks `deep_processing: yes` should continue to the staged workflow.
